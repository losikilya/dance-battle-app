...
const ServerRow: React.FC<{ label: string; value: string }> = ({ label, value }) => (
...
export const ServerStatusCard: React.FC = () => {
  ...
  const localIp = useJudgingServerStore(s => s.localIp);
  ...
  return (
    <Box style={styles.card} p={20} gap={16}>
      <Box direction="row" justify="space-between" align="center">
        ...
        <Box style={{ ...styles.badge, backgroundColor: isOnline ? Colors.status.online : Colors.text.secondary }} px={8} py={4}>
          <Text variant="body2" color="dark">{isOnline ? getResource('dashboard_server_online') : getResource('dashboard_server_offline')}</Text>
        </Box>
      </Box>
      <Box gap={8}>
